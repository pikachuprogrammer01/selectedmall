...
const isSelected = computed(() => {
  return cartItems.value.some(item => selectedProducts.value.includes(item.productId))
})
...
const selectAll = computed({
  get: () => {
    return cartItems.value.length > 0 && selectedProducts.value.length === cartItems.value.length
  },
  set: (value) => {
    if (value) {
    ...
    <div class="cart-content" v-if="cartItems.length > 0">
        ...
        <div class="cart-item" v-for="item in cartItems" :key="item.productId">
          ...
          <div class="item-actions">
            <el-input-number
              v-model="item.count"
              :min="1"
              @change="(val) => handleQuantityChange({ productId: item.productId, count: val })"
            />
          </div>
          ...
          <div class="item-remove">
            <el-button type="danger" text @click="handleRemoveProduct(item.productId)">
              删除
            </el-button>
          </div>
        </div>
      ...
      <div class="cart-summary">
        <div class="summary-header">
          ...
          <span>全选</span>
          <el-button type="danger" text @click="handleClearCart" :disabled="!isSelected">
            清空
          </el-button>
        </div>
        ...
        <div class="summary-content">
          ...
          <el-button type="primary" size="large" @click="handleCheckout" :disabled="!isSelected">
            去结算
