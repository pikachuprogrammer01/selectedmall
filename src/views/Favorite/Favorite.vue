...
const totalAmount = computed(() => {
  return selectedProducts.value.reduce((total, item) => total + item.price, 0)
})
...
const handleRemoveSelected = () => {
  ...
  ElMessageBox.confirm(`确定要移除选中的 ${selectedIds.value.length} 个商品吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    favorites.value = favorites.value.filter(fav => !selectedIds.value.includes(fav.id))
    selectedIds.value = []
  ...
  <div class="favorite">
    ...
    <div class="favorite-content" v-if="favorites.length > 0">
      <div class="favorite-list">
        <div class="favorite-item" v-for="product in favorites" :key="product.id">
          <el-checkbox
    ...
    <div class="bottom-bar" v-if="favorites.length > 0">
      <div class="select-all">
        <el-checkbox
          :model-value="favorites.length > 0 && selectedIds.length === favorites.length"
